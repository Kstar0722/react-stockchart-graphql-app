/*
	in order to modify Annotate.js

	copied it from node_modules to src folder
	modified it
*/
	...
	_createClass(Annotate, [{
		...
		value: function renderSVG(moreProps) {
			...
			var xScale = moreProps.xScale,
			    ...
			    plotData = moreProps.plotData; /* data is stored in this variable */
			var _props = this.props,
			    className = _props.className,
			    usingProps = _props.usingProps,
				Annotation = _props.with,
				indicatorName = _props.usingProps.indicatorName, /* insert 'indicatorName' props to Annotate */
				color = _props.usingProps.color /* insert 'color' props to Annotate */
			...
			return _react2.default.createElement(
				...
				data.map(function (d, idx) {
					...
					usingProps.text = data[idx].indicators[ indicatorName ] /* set text according to the matching */

					/*
						if color is null, set red on top and green on bottom
						if color is not null, use it
					*/
					if( color === null ) {
						if( data[idx].close > data[idx].open ) {
							usingProps.fill = "#DB0000"
						} else {
							usingProps.fill = "#6BA583"
						}
					} else {
						usingProps.fill = color
					}
